...
#port option is pretty much ignored now. -sartan
options.add_option('-p', '--ports', default=443, help='TCP ports (comma-seperated) to scan with nmap and test (default: 443)')
...
def hit_hb(s):
    ...
    while True:
        ...
        if typ == 24:
            ...
            hexdump(pay)
            if len(pay) > 3:
                print '\033[1mWARNING: server returned more data than it should - server is vulnerable!'
            else:
...
def scanhost(host,port,starttls):
...
def startscan(hosts,ports,starttls):
  nm = nmap.PortScanner()
  nm.scan(hosts,str(ports), arguments='--open -Pn')
  for ip in nm.all_hosts():
    for port in nm[ip]['tcp'].keys():
      print "Scanning",ip,port
      scanhost(ip,port,starttls)
...
def main():
    ...
    if len(args) < 1:
        options.print_help()
        return
    startscan(args[0],opts.ports,opts.starttls)
